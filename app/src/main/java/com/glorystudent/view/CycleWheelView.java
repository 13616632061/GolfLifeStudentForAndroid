...
import com.glorystudent.golflife.TPIEvaluationActivity;
...
public class CycleWheelView extends ListView {
    ...
    private int mImageLabeIvId;
    //1，主问题，2，小问题，3，选项
    private int mType;
    private List<TestModle> mTestlist;
    private List<OptionModle> mOptionList;
    private List<ItemModle> mItemList;
    private int mImageRotateId;
    ...
    private void resetItems(int firstPosition, int position, int offset){
        for (int i = position - offset - 1; i < position + offset + 1; i++) {
            ...
            ImageView imageView = (ImageView) itemView.findViewById(mImageLabeIvId);
            ImageView imageRotateView = (ImageView) itemView.findViewById(mImageRotateId);

            TextView labelTv = (TextView) itemView.findViewById(mItemLabelTvId);
            if (position == i) {
                labelTv.setTextColor(mLabelSelectColor);
                itemView.setAlpha(1f);
                imageView.setImageResource(R.drawable.btnhomechoiceorange);
                if (mType==1) {
                    if (mTestlist.get(getSelection()).isROTATE()){
                        Animation operatingAnim = AnimationUtils.loadAnimation(getContext(), R.anim.activity_tpi);
                        LinearInterpolator lin = new LinearInterpolator();
                        operatingAnim.setInterpolator(lin);
                        imageRotateView.setVisibility(VISIBLE);
                        if (operatingAnim != null) {
                            imageRotateView.startAnimation(operatingAnim);
                        }
                    }else {
                        imageRotateView.setVisibility(GONE);
                    }
                ...
                imageView.setImageResource(R.drawable.btnhomechoiceblack);
                imageRotateView.clearAnimation();
                imageRotateView.setVisibility(GONE);
            }
     ...
     * @param list
     */
    public void setLabels(List<TestModle> list) {
        mTestlist = list;
    ...
    public void setLabels1(List<OptionModle> list) {
        mOptionList = list;
        List<String> labels=new ArrayList<>();
        for (int i=0;i<list.size();i++){
            labels.add(list.get(i).getName());
    ...
    public void setLabels2(List<ItemModle> list) {
        mItemList = list;
        List<String> labels=new ArrayList<>();
        for (int i=0;i<list.size();i++){
            labels.add(list.get(i).getName());
     ...
     ** type 1，主问题，2，小问题，3，选项
     */
    public void setWheelItemLayout(int itemResId, int labelTvId,int imageId,int imageRotateId,int type) {
        mImageRotateId = imageRotateId;
        mType = type;
        mItemLayoutId = itemResId;
        mItemLabelTvId = labelTvId;
        mImageLabeIvId = imageId;
        mAdapter = new CycleWheelViewAdapter();
        mAdapter.setData(mLabels);
        setAdapter(mAdapter);
        initView();
    ...
    public class CycleWheelViewAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            ImageView imageView = (ImageView) convertView.findViewById(mImageLabeIvId);

            ImageView imageRotateView = (ImageView) convertView.findViewById(mImageRotateId);
            switch (mType){
                case 1:
                    if (position!=0&&position!=mTestlist.size()+1){
                        if (mTestlist.get(position-1).isFlag()){
                            imageView.setVisibility(VISIBLE);
                            mTestlist.get(position-1).setROTATE(false);
                            imageRotateView.clearAnimation();
                            imageRotateView.setVisibility(GONE);
                        }else{
                            imageRotateView.clearAnimation();
                    ...
                    break;
                case 2:
                    if (position!=0&&position!=mOptionList.size()+1){
                        if (mOptionList.get(position-1).isFlag()){
                            imageView.setVisibility(VISIBLE);
                        }else {
                            imageView.setVisibility(GONE);
                        }
                ...
                case 3:
                    ...
                    textView.setOnClickListener(new OnClickListener() {
                        ...
                        public void onClick(View view) {
                            TPIEvaluationActivity.cycleWheelCallBack.itemClickCallback(position-1);
