...
public class TeamMemberDecoration extends RecyclerView.ItemDecoration {
    private static final String TAG = "TeamMemberDecoration";
    ...
    public TeamMemberDecoration(List<LetterEntity> dataList, Context context, DecorationCallback decorationCallback) {
        Resources res = context.getResources();
        this.dataList = dataList;
        this.callback = decorationCallback;
        //设置悬浮栏的画笔---paint
        paint = new Paint();
        paint.setColor(res.getColor(R.color.colorGray5));

        //设置悬浮栏中文本的画笔
        textPaint = new TextPaint();
        textPaint.setAntiAlias(true);
        textPaint.setTextSize(DensityUtil.sp2px(context, 12));
        textPaint.setColor(res.getColor(R.color.colorGray));
        textPaint.setTextAlign(Paint.Align.LEFT);
        fontMetrics = new Paint.FontMetrics();
        //决定悬浮栏的高度等
        topGap = DensityUtil.dip2px(context, 22);
        //决定文本的显示位置等
        alignBottom = DensityUtil.dip2px(context, 5);
    }
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (pos == 0 || isFirstInGroup(pos)) {
            outRect.top = topGap;
//            if (dataList.get(pos).getName().isEmpty()) {
//                outRect.top = 0;
//            }
        } else {
            outRect.top = 0;
